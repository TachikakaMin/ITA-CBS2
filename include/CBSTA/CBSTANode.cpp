...
CBSTANode::CBSTANode(shared_ptr<CBSTANode> curnode) {
...
CBSTANode::CBSTANode(int n){
    is_root = false;
    constraint_sets.resize(n, shared_ptr<Constraints>(new Constraints));
}


void CBSTANode::get_path_from_TA(vector<vector<shared_ptr<Path > > >& cost_matrix)
{
...
bool CBSTANode::get_first_conflict(Conflict &result) {
