...
public:
    ...
    Timer ta_timer, total_timer, lowlevel_search_timer, firstconflict_timer, newnode_timer, conflict_num_timer, focal_score_timer;
    double ta_runtime, total_runtime,  lowlevel_search_time, firstconflict_time, newnode_time, conflict_num_time, focal_score_time;
    ...
    shared_ptr<Path> findPath_a_star_eps(shared_ptr<Constraints>&  agent_constraint_set, int agent_idx, int goal_loc_idx, const vector<shared_ptr<Path > >& out_solution, vector<vector<int> >& fmin);
    shared_ptr<Path> findPath_a_star(shared_ptr<Constraints>& agent_constraint_set, int agent_idx, int goal_loc_idx, vector<vector<int> >& fmin);
