                                               ...
                                               vector<vector<int> >& fmin)
{
    ...
    while (!openSet.empty() && search_max > 0) {
        ...
        {
            fmin[agent_idx][goal_loc_idx] = best_fScore;
    ...
    fmin[agent_idx][goal_loc_idx] = INF7f;
    return nullptr;
}


shared_ptr<Path> ITA_ECBS::findPath_a_star(shared_ptr<Constraints>& agent_constraint_set,
                                           int agent_idx, int goal_loc_idx, vector<vector<int> >& fmin)
{
    this->invoke_shortest_path_times ++;
    int search_max = 1e5;
    int m_lastGoalConstraint = -1;
    ...
    while (!openSet.empty() && search_max > 0) {
        ...
        if (current->state.x == goal_location.x && current->state.y == goal_location.y && current->state.time > m_lastGoalConstraint)
        {
            fmin[agent_idx][goal_loc_idx] = current->fScore;
            shared_ptr<Path> path(new Path);
            while (current != nullptr) {
                path->push_back(PathEntry(current->state, current->fScore, current->gScore, nullptr));
                current = current->parent;
...
//        printf("cur_node->focal_score: %d\n", cur_node->focal_score);
        ...
        for (unsigned short cur_i = 0; auto &[key, value]: tmp)
        {
            ...
            cur_i ++;
            new_node->update_cost_matrix(this, key, value);
            this->num_ta ++;
            this->ta_timer.reset();
            bool b2 = new_node->get_next_assignment(key, this->l_weight, changed_agent);
            this->ta_timer.stop();
