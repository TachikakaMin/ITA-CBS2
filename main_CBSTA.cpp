//
// Created by YIMIN TANG on 5/18/23.
//
#include <iostream>
...
#include "include/CBSTA/CBSTA.hpp"
...
int main(int argc, char** argv) {
    ...
    CBSTA cbsta(row_number, col_number, obstacles, goals, start_states, goal_to_idx, idx_to_goal);

    int runs = vm["restart"].as<int>();
    for (int i = 0; i < runs; i++) {
        cbsta.clear();
        cbsta.total_timer.reset();
        cbsta.solve();
        cbsta.total_timer.stop();
        cbsta.total_runtime = cbsta.total_timer.elapsedSeconds();
        if (cbsta.solution_found) break;
    }
    std::ofstream out(outputFile);
    out << "statistics:" << std::endl;
    out << "  cost: " << cbsta.cost << std::endl;
    out << "  TA_runtime: " << cbsta.ta_runtime << std::endl;
    out << "  newnode_runtime: " << cbsta.newnode_time << std::endl;
    out << "  firstconflict_runtime: " << cbsta.firstconflict_time << std::endl;
    out << "  runtime: " << cbsta.total_runtime << std::endl;
    out << "  lowlevel_search_time: " << cbsta.lowlevel_search_time << std::endl;
    out << "  total_lowlevel_node: " << cbsta.cbsnode_num << std::endl;
    out << "  lowLevelExpanded: " << cbsta.lowLevelExpanded << std::endl;
    out << "  numTaskAssignments: " << cbsta.num_ta << std::endl;

    out << "schedule:" << std::endl;
    for (size_t a = 0; a < cbsta.out_solution.size(); ++a) {
        out << "  agent" << a << ":" << std::endl;
        for (const auto &state: *(cbsta.out_solution[a])) {
